...
bool Robot::loadParams(rclcpp::Node::SharedPtr node) {
    ...
    if (node->get_parameter("vehicle_config", vehicle_config_file) &&
        ...
        try {
            RCLCPP_INFO(node->get_logger(), "Opening vehicle config file: %s and simulator config file %s",
...
void Robot::storeConfigData(YAML::Node vehicle_config, YAML::Node simulator_config) {
    ...
    const std::string hydroFile = node->declare_parameter<std::string>("hydrodynamics_config", "");
    if (hydroFile.empty())
        throw std::invalid_argument("hydrodynamics_config is required; use the physics launch or supply a "
                                    "validated model YAML");
    const auto hydro = YAML::LoadFile(hydroFile);
    if (hydro["robot"].as<std::string>() != name)
        throw std::invalid_argument("Hydrodynamic model robot does not match namespace");
    if (hydro["schema_version"].as<int>() != 1)
        throw std::invalid_argument("Unsupported hydrodynamic schema");
    if (hydro["parameter_status"].as<std::string>("unvalidated_prior") != "identified")
        RCLCPP_WARN(node->get_logger(), "Hydrodynamic coefficients are UNVALIDATED PRIORS; pool "
    ...
    currentAmplitude = std2v3d(hydro["current_oscillation_amplitude"].as<std::vector<double>>());
    ...
    // base_link.
    r_baseLink = std2v3d(vehicle_config["base_link"].as<std::vector<double>>()) - r_com;
    for (const auto &camera : vehicle_config["cameras"]) {
        ...
        const v3d mount = std2v3d(pose) - std2v3d(vehicle_config["base_link"].as<std::vector<double>>());
    ...
    imu_rate = 1.0 / sensorProperties["imu_rate"].as<double>(vehicle_config["imu"]["rate"].as<double>());
    ...
    imu_orientationVariance = sensorProperties["imu_orientation_variance"]
                                  ? std2v3d(sensorProperties["imu_orientation_variance"].as<std::vector<double>>())
                                  : v3d::Constant(imu_sigmaAngle * imu_sigmaAngle);
    imu_angularVelocityVariance =
        sensorProperties["imu_angular_velocity_variance"]
            ? std2v3d(sensorProperties["imu_angular_velocity_variance"].as<std::vector<double>>())
    ...
    imu_linearAccelerationVariance =
        sensorProperties["imu_linear_acceleration_variance"]
            ? std2v3d(sensorProperties["imu_linear_acceleration_variance"].as<std::vector<double>>())
    ...
    std::vector<c_simulator::ThrusterParameters> actuatorParameters(thrusterCount);
    ...
    actuator.configure(actuatorParameters, dynamics["command_timeout"].as<double>(.5));
    // Thruster info
    int row = 0;
    ...
    if (ACOUSTIC_DATA) {
        ...
        std::vector<double> fakePingerPose =
            simulator_config["acoustics"]["fake_pinger"]["pose"].as<std::vector<double>>();
...
vXd Robot::stateDerivative(const vXd &x, double stageOffset) const {
    const double phase = 2 * M_PI * currentFrequency * (actuator.time() + stageOffset);
    return marineDynamics.derivative(x, propulsionWrench(x), waterCurrentWorld + currentAmplitude * sin(phase),
...
c_simulator::Vector6d Robot::propulsionWrench(const vXd &x) const {
    ...
    for (int i = 0; i < thrusterCount; ++i) {
        ...
        double extent = std::max(.001, propellerRadius * sqrt(std::max(0., 1 - axisZ * axisZ)));
...
void Robot::setAccel(const vXd &stateDot) {
    ...
    linAccel = q * (stateDot.segment(7, 3) + bodyOmega.cross(bodyVelocity)) + v3d(0, 0, GRAVITY);
...
geometry_msgs::msg::Transform Robot::getLCameraTransform() {
    if (!hasLCameraTransform) {
        ...
        geometry_msgs::msg::Transform mount = safeTransform(toFrameRel, fromFrameRel, hasLCameraTransform);
        if (hasLCameraTransform)
...
v3d Robot::getWaterCurrentWorld() {
    return waterCurrentWorld;
}
...
v3d Robot::getIMUOrientationVariance() {
    return imu_orientationVariance;
}
...
double Robot::getAcousticsPingTime() {
    ...
    double portDistance = sqrt(pow(portTranslation_v3d[0] - fakePingerPosition[0], 2) +
                               pow(portTranslation_v3d[1] - fakePingerPosition[1], 2) +
                               pow(portTranslation_v3d[2] - fakePingerPosition[2], 2));
    double starboardDistance = sqrt(pow(starboardTranslation_v3d[0] - fakePingerPosition[0], 2) +
...
geometry_msgs::msg::Transform Robot::safeTransform(string toFrame, string fromFrame, bool &transformFlag) {
    ...
    try {
        geometry_msgs::msg::TransformStamped t = tf_buffer->lookupTransform(toFrame, fromFrame, tf2::TimePointZero);
        transformFlag = true;
        return t.transform;
    } catch (const tf2::TransformException &ex) {
        RCLCPP_ERROR_SKIPFIRST_THROTTLE(node->get_logger(), *node->get_clock(), 2000,
                                        "Could not transform %s to %s: %s", toFrame.c_str(), fromFrame.c_str(),
                                        ex.what());
