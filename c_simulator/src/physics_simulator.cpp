...
public:
  ...
  PhysicsSimNode() : Node("physics_simulator") {
    ...
    if (!std::isfinite(realTimeFactor) || realTimeFactor <= 0 ||
        !std::isfinite(clockPublishRate) || clockPublishRate <= 0)
    ...
    gyroVariance = declare_parameter<double>(
        "gyro_variance", std::max(1e-9, gyroSigma * gyroSigma));
    if (!std::isfinite(gyroRate) || gyroRate <= 0 ||
        !std::isfinite(gyroSigma) || gyroSigma < 0 ||
    ...
    if (!std::isfinite(physicsStep) || physicsStep <= 0 || physicsStep > .02 ||
        !std::isfinite(contactFriction) || contactFriction < 0 ||
    ...
    transformListener =
        std::make_shared<tf2_ros::TransformListener>(*transformBuffer);
    ...
    clockPub = create_publisher<rosgraph_msgs::msg::Clock>(
        "/clock", rclcpp::QoS(rclcpp::KeepLast(10)));
    gyroPub = create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>(
        "gyro/twist", 10);
    firmwareKillPub =
        this->create_publisher<std_msgs::msg::Bool>("state/kill", 10);
    truthPub =
        create_publisher<nav_msgs::msg::Odometry>("simulator/ground_truth", 10);
    actualThrusterPub = create_publisher<std_msgs::msg::Float32MultiArray>(
        "simulator/actual_thruster_forces", 10);
    timePub = create_publisher<std_msgs::msg::Float64>("simulator/time", 10);
    statePub =
        this->create_publisher<geometry_msgs::msg::Pose>("simulator/state", 10);
    dvlPub =
    ...
    depthPub =
        this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
            "depth/pose", 10);
    acousticsPub = this->create_publisher<geometry_msgs::msg::Vector3Stamped>(
        "acoustics/delta_t", 10);
    collisionBoxPub =
        this->create_publisher<visualization_msgs::msg::MarkerArray>(
            "simulator/collisionMarkers", 10);
    thrusterTelemetryPub =
        this->create_publisher<riptide_msgs2::msg::DshotPartialTelemetry>(
            "state/thrusters/telemetry", 10);
    thrusterSub = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "thruster_forces", 10,
        std::bind(&PhysicsSimNode::forceCallback, this, _1));
    softwareKillSub =
        this->create_subscription<riptide_msgs2::msg::KillSwitchReport>(
            "command/software_kill", 10,
            std::bind(&PhysicsSimNode::killSwitchCallback, this, _1));

    clawJointsSub=create_subscription<std_msgs::msg::Float64MultiArray>("simulator/claw_joints",10,
      [this](const std_msgs::msg::Float64MultiArray& m){if(taskContacts&&m.data.size()==2)taskContacts->setJaws(m.data[0],m.data[1]);});
    taskObjectsSub=create_subscription<visualization_msgs::msg::MarkerArray>("simulator/task_objects",10,
      [this](const visualization_msgs::msg::MarkerArray& m){if(!taskContacts)return;
        for(const auto& marker:m.markers){if(marker.action!=visualization_msgs::msg::Marker::ADD)continue;
          const auto& p=marker.pose.position;const auto& r=marker.pose.orientation;
          Eigen::Isometry3d t=Eigen::Isometry3d::Identity();t.translate(v3d(p.x,p.y,p.z));t.rotate(quat(r.w,r.x,r.y,r.z).normalized());
          const bool attached=marker.header.frame_id==robot.getName()+"/base_link";
          if(attached)t.pretranslate(robot.getBaseLinkOffset());
    ...
    ekfOdomSub = create_subscription<nav_msgs::msg::Odometry>(
        "odometry/filtered", 10, [this](const nav_msgs::msg::Odometry &odom) {
    ...
    syncService = create_service<std_srvs::srv::Trigger>(
        "sync_sim_to_estimate",
        std::bind(&PhysicsSimNode::syncSimToEstimate, this, _1, _2));
    resetStartService = create_service<std_srvs::srv::Trigger>(
        "reset_sim_to_start",
        std::bind(&PhysicsSimNode::resetSimToStart, this, _1, _2));
    ekfAlignmentTimer = create_wall_timer(
        100ms, std::bind(&PhysicsSimNode::alignEkf, this));
  ...
  bool init() {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Loading robot's parameter data from YAML..");
    bool paramsLoaded = robot.loadParams(shared_from_this());
    if (!paramsLoaded)
      return false;
    dvlSigma = declare_parameter<double>("dvl_noise_stddev", robot.getDVLSigma());
    dvlVariance = declare_parameter<double>(
        "dvl_variance", std::max(1e-9, dvlSigma * dvlSigma));
    ...
    if (!std::isfinite(dvlSigma) || dvlSigma < 0 ||
        !std::isfinite(dvlVariance) || dvlVariance <= 0 ||
        !std::isfinite(imuGravity) || imuGravity <= 0 ||
        !std::isfinite(imuYawDrift))
    ...
    bool collisionBoxesLoaded =
        (COLLISION_TOGGLE ? loadCollisionFiles() : true);
    ...
    if (paramsLoaded && collisionBoxesLoaded) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Initization successful. Sensors scheduled on simulated time");
  ...
  void rungeKutta4() {
    ...
    while (rclcpp::ok()) {
      ...
      accumulator += realTimeFactor *
                     std::chrono::duration<double>(currentTime - previousTime)
                         .count();
      ...
      while (accumulator >= physicsStep && steps < MAX_CATCHUP_STEPS) {
        ...
        if (collisionsEnabled) advanced = handleCollisions(advanced);
      ...
      // steps.
      const double remaining = (physicsStep - accumulator) / realTimeFactor;
      if (remaining > 0.0002)
        std::this_thread::sleep_for(
            std::chrono::duration<double>(remaining - 0.0001));
    }
...
private:
  ...
  vXd handleCollisions(vXd state) {
    ...
    for (collisionBox &robotBox : robotBoxes) {
      ...
      for (collisionBox obstacleBox : obstacleBoxes) {
        ...
        if (collision.collided) {
          RCLCPP_DEBUG(this->get_logger(), "%s has collided with %s",
                       robot.getName().c_str(), obstacleBox.getName().c_str());
          // Compute impulse
          ...
          double v_rel =
              (q * state.segment<3>(7) + (q * state.segment<3>(10)).cross(r_a))
          ...
          if (v_rel < 0.0) {
            ...
            const double effectiveInverseMass =
                contactJacobian.dot(inverseMass * contactJacobian);
            if (effectiveInverseMass > 1e-12) {
              const double impulse =
                  -(1.0 + restitution) * v_rel / effectiveInverseMass;
              state.segment(7, 6) += inverseMass * contactJacobian * impulse;
              const v3d contactVelocity =
                  state.segment<3>(7) +
                  state.segment<3>(10).cross(contactOffsetBody);
              v3d tangent = contactVelocity -
                            normalBody * contactVelocity.dot(normalBody);
              if (tangent.norm() > 1e-9) {
                ...
                const double friction =
                    std::min(contactFriction * impulse,
                             jt.dot(state.segment<6>(7)) / jtInv);
    ...
    if (taskContacts) state=taskContacts->resolve(state,robot.getInverseMass(),contactFriction);
    return state;
  }
  ...
  bool loadCollisionFiles() {
    ...
    RCLCPP_INFO(get_logger(), "Loading scene from file: %s",
                _sceneFile.c_str());
    YAML::Node sceneFile = YAML::LoadFile(_sceneFile);
    if (!sceneFile) {
      RCLCPP_ERROR(this->get_logger(), "Failed to load scene file: %s",
                   _sceneFile.c_str());
      return false;
    }

    const auto mapping =
        YAML::LoadFile(declare_parameter<std::string>("mapping_config", ""));
    const auto data = mapping["/" + robot.getName() + "/riptide_mapping2"]
                             ["ros__parameters"]["init_data"];
    if (!data)
      throw std::runtime_error("Mapping config has no robot init_data");
    std::map<std::string, Eigen::Isometry3d> frames;
    std::set<std::string> visiting;
    std::function<Eigen::Isometry3d(std::string)> resolve =
        [&](std::string key) -> Eigen::Isometry3d {
      ...
      local.translate(v3d(pose["x"].as<double>(0), pose["y"].as<double>(0),
                          pose["z"].as<double>(0)));
      local.rotate(
          Eigen::AngleAxisd(deg2rad(pose["yaw"].as<double>(0)), v3d::UnitZ()));
      Eigen::Isometry3d result =
          resolve(entry["parent"].as<std::string>()) * local;
      visiting.erase(key);
      frames[key] = result;
      return result;
    };
    const auto origin =
        mapping["/**/zed_faker"]["ros__parameters"]["map_origin_pool"]
    ...
    if (sceneFile["robot"]["collision"].IsDefined()) {
      std::string robotCollisionPath =
          ...
          sceneFile["robot"]["collision"].as<string>();
      urdf::ModelInterfaceSharedPtr robotModel =
          urdf::parseURDFFile(robotCollisionPath);
      if (!robotModel) {
    ...
    for (auto const &entryPair : sceneFile["objects"]) {
      ...
      if (!data[key]) {
        RCLCPP_WARN(get_logger(),
                    "No mapping pose for %s; skipping legacy collision proxy",
                    key.c_str());
        continue;
      }
      ...
      std::vector<collisionBox> newBoxes =
          unpackURDF(objectModel, position, objQuat);
    ...
    if (!taskFile.empty()) {
      ...
      const double inner = crate["inner_width"].as<double>() -
                           2 * crate["liner_thickness"].as<double>();
      ...
      for (int i = 1; i <= 4; ++i) {
        ...
        auto add = [&](const v3d &p, const v3d &size) {
          obstacleBoxes.emplace_back(key + "_crate", size.x(), size.y(),
                                     size.z(), t * p, v3d::Zero(),
                                     quat(t.rotation()));
        };
        add({0, 0, -base / 2}, {outer, outer, base});
        for (int sign : {-1, 1}) {
          add({sign * (outer - wall) / 2, 0, height / 2},
              {wall, outer, height});
          add({0, sign * (outer - wall) / 2, height / 2},
              {outer, wall, height});
        }
      }
    }
    // Pool planes share the exact corner-origin transform used by the renderer.
    auto poolBox = [&](const std::string &name, const v3d &size,
                       const v3d &center) {
      obstacleBoxes.emplace_back(name, size.x(), size.y(), size.z(),
                                 poolToMap * center, v3d::Zero(),
                                 quat(poolToMap.rotation()));
    };
    poolBox("floor", v3d(50, 22.86, 1), v3d(25, 11.43, -2.6336));
    poolBox("pool_west_wall", v3d(1, 24.86, 3.4386), v3d(-.5, 11.43, -.9143));
    poolBox("pool_east_wall", v3d(1, 24.86, 3.4386), v3d(50.5, 11.43, -.9143));
    poolBox("pool_south_wall", v3d(50, 1, 3.4386), v3d(25, -.5, -.9143));
    poolBox("pool_north_wall", v3d(50, 1, 3.4386), v3d(25, 23.36, -.9143));
    if(!taskFile.empty()){
      const auto task=YAML::LoadFile(taskFile);
      if(task["octagon"] && data["octagon"]){
        const auto cfg=task["octagon"];
        auto mount=resolve("octagon");mount.translation().z()=cfg["surface_z"].as<double>();
        ...
        for(int i=0;i<8;++i){
          ...
          obstacleBoxes.emplace_back("octagon_pvc",length,2*r,2*r,t.translation(),v3d::Zero(),quat(t.rotation()));
        }
      }
      if(task["claw"]){
        const auto vehicle=YAML::LoadFile(get_parameter("vehicle_config").as_string());
        const auto c=vehicle["com"].as<std::vector<double>>();
        taskContacts=std::make_unique<TaskContacts>(task,(collisionFolder/"tasks").string(),v3d(c[0],c[1],c[2]),resolve);
        auto addBox=[&](collisionBox& box,bool attached){Eigen::Isometry3d t=Eigen::Isometry3d::Identity();t.translate(box.getCenter());t.rotate(box.getOrientation());
          taskContacts->addBox(box.getName(),v3d(box.getLength(),box.getWidth(),box.getHeight()),t,attached);};
        for(auto& box:obstacleBoxes)addBox(box,false);
        for(auto& box:robotBoxes)addBox(box,true);
      }
    }
    return true;
  }
   ...
   */
  std::vector<collisionBox> unpackURDF(urdf::ModelInterfaceSharedPtr model,
                                       v3d basePosition, quat baseOrientation) {
    std::vector<collisionBox> newBoxes;
    ...
    for (const auto &link_pair : model->links_) {
      ...
      for (const auto &collision : link->collision_array) {
        ...
        // Add box to list
        collisionBox newBox =
            collisionBox(link->name, length, width, height, basePosition,
                         baseOffset, baseOrientation, baseOrientationOffset);
  ...
  void publishCollisionMarkers() {
    if (collisionBoxPub->get_subscription_count() == 0) return;
    ...
    combinedBoxList.insert(combinedBoxList.begin(), robotBoxes.begin(),
                           robotBoxes.end());
  ...
  void publishFakeDepthData() {
    ...
    double depthData =
        (state.head<3>() + state2quat(state) * robot.getDepthOffset()).z();
    ...
    depthMsg.pose.pose.position.z =
        depthData + (state2quat(state) *
                     (robot.getBaseLinkOffset() - robot.getDepthOffset()))
                        .z();
    depthMsg.pose.pose.orientation.w = 1.;
    depthMsg.pose.covariance[14] =
        robot.getDepthSigma() * robot.getDepthSigma();
  ...
  void publishFakeGyroData() {
    const v3d omega = robot.getGyroQuat().conjugate() *
                      robot.getState().segment<3>(10);
    geometry_msgs::msg::TwistWithCovarianceStamped msg;
    msg.header.stamp = simNow();
    msg.header.frame_id = robot.getName() + "/fog_link";
    msg.twist.twist.angular.z = noiseEnabled && !sync_odom
                                  ? randomNorm(omega.z(), gyroSigma) : omega.z();
  ...
  void publishFakeIMUData() {
    if (robot.imuTransformAvailable()) {
      ...
      angularVel =
          robot.getIMUQuat().conjugate() * (q.conjugate() * angularVel);
      ...
      if (noiseEnabled && !this->sync_odom) {
        ...
        v3d randomAxis(unitNormal(randomGenerator), unitNormal(randomGenerator),
                       unitNormal(randomGenerator));
        randomAxis.normalize();
        std::normal_distribution<double> angleNoise(0.0, imu_sigma[2]);
        Eigen::AngleAxisd randomAngleNoise(angleNoise(randomGenerator),
                                           randomAxis);
  ...
  void publishFakeAcousticsData() {
    ...
    if (robot.acousticsTransformAvailable() && robot.mapAvailable() &&
        ACOUSTIC_DATA) {
  ...
  void forceCallback(const std_msgs::msg::Float32MultiArray &thrusterForces) {
    if (static_cast<int>(thrusterForces.data.size()) !=
        robot.getThrusterCount()) {
      RCLCPP_ERROR_THROTTLE(
          get_logger(), *get_clock(), 2000,
          "Ignoring %zu thruster forces; vehicle has %d thrusters",
          thrusterForces.data.size(), robot.getThrusterCount());
      return;
    }
    for (float force : thrusterForces.data)
      if (!std::isfinite(force)) {
        RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 2000,
                              "Ignoring nonfinite thruster command");
        return;
      }
    // Create a zero vector with the current time
    thrusterForcesStamped commandedThrust(vXd::Zero(robot.getThrusterCount()),
                                          simNow().seconds());
  ...
  void killSwitchCallback(
      const riptide_msgs2::msg::KillSwitchReport &softwareKillMsg) {
  ...
  // requested position
  void setSim(const std::shared_ptr<robot_localization::srv::SetPose::Request>
                  poseRequest) {
    auto desired = poseRequest->pose;
    if (desired.header.frame_id.empty())
      desired.header.frame_id = "map";
    auto &pose = desired.pose.pose;
    quat q(pose.orientation.w, pose.orientation.x, pose.orientation.y,
           pose.orientation.z);
    ...
    if (desired.header.frame_id != "map") {
      try {
        auto t = transformBuffer
                     ->lookupTransform("map", desired.header.frame_id,
                                       tf2::TimePointZero)
                     .transform;
        quat rotation(t.rotation.w, t.rotation.x, t.rotation.y, t.rotation.z);
        position = rotation * position +
                   v3d(t.translation.x, t.translation.y, t.translation.z);
        q = rotation * q;
  ...
  void resetSimToStart(
    ...
    std::snprintf(text, sizeof text,
                  "Simulator reset to start base_link (%.2f, %.2f, %.2f) m, yaw %.1f deg",
  ...
  void syncSimToEstimate(
    ...
    q.normalize();
    const string frame =
        latestEkfOdom.header.frame_id.empty() ? ekfFrame : latestEkfOdom.header.frame_id;
    if (frame != "map") {
      try {
        const auto t = transformBuffer
                           ->lookupTransform("map", frame, tf2::TimePointZero)
                           .transform;
        const quat rotation(t.rotation.w, t.rotation.x, t.rotation.y,
                            t.rotation.z);
        position = rotation * position +
                   v3d(t.translation.x, t.translation.y, t.translation.z);
        q = rotation * q;
    ...
    char text[160];
    std::snprintf(text, sizeof text,
                  "Simulator moved to EKF base_link (%.2f, %.2f, %.2f) m, yaw %.1f deg",
                  position.x(), position.y(), position.z(), yaw * 180 / M_PI);
    response->message = text;
    response->success = true;
    RCLCPP_INFO(get_logger(), "%s", text);
  ...
  void alignEkf() {
    ...
    if (!ekfAlignmentPending || ekfFrame.empty() ||
        !poseClient->service_is_ready())
      return;
    const auto state = robot.getState();
    quat q = state2quat(state);
    ...
    if (ekfFrame != "map") {
      try {
        const auto t = transformBuffer
                           ->lookupTransform(ekfFrame, "map", tf2::TimePointZero)
                           .transform;
        const quat rotation(t.rotation.w, t.rotation.x, t.rotation.y,
                            t.rotation.z);
        position = rotation * position +
    ...
    RCLCPP_INFO(get_logger(), "Initialized EKF from simulator base_link in %s",
                ekfFrame.c_str());
  }
  ...
  void publishState() {
    {
      ...
      cameraFrameL.child_frame_id =
          "simulator/" + robot.getName() + "/ffc_camera_link";
      cameraOpticalFrameL.header.frame_id = cameraFrameL.child_frame_id;
      cameraOpticalFrameL.child_frame_id =
          "simulator/" + robot.getName() + "/ffc_left_camera_optical_frame";
      ...
      const v3d baseVelocity =
          state.segment<3>(7) +
          state.segment<3>(10).cross(robot.getBaseLinkOffset());
  ...
  rclcpp::Time simNow() const { return rclcpp::Time(simNs, RCL_ROS_TIME); }
  ...
  void schedule(double periodSeconds, std::function<void()> job) {
    const int64_t period =
        std::max<int64_t>(stepNs, std::llround(periodSeconds * 1e9));
  ...
  void refreshRealTimeFactor() {
    ...
    if (!std::isfinite(requested) || requested <= 0) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           "Ignoring invalid real_time_factor %g", requested);
      return;
    }
    RCLCPP_INFO(get_logger(), "real_time_factor %.3g -> %.3g", realTimeFactor,
                requested);
  ...
  rclcpp::Subscription<riptide_msgs2::msg::KillSwitchReport>::SharedPtr
      softwareKillSub;
  rclcpp::Publisher<riptide_msgs2::msg::DshotPartialTelemetry>::SharedPtr
      thrusterTelemetryPub;
};
...
int main(int argc, char *argv[]) {
  ...
  if (startUpSuccess) {
    ...
    RCLCPP_FATAL(node->get_logger(),
                 "SIM FAILED TO START: Could not initialize");
    rclcpp::shutdown();
    return 1;
  }
  return 0;
