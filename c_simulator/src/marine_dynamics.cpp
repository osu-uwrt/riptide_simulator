...
void positiveSemidefinite(const Matrix &m, const char *name) {
...
void MarineDynamics::configure(double mass, const Eigen::Matrix3d &inertia,
                               const Matrix6d &added) {
  ...
  if (eig.eigenvalues().minCoeff() <= 0 ||
    ...
    throw std::invalid_argument(
        "Rigid inertia must be positive definite and satisfy the "
...
void MarineDynamics::configureDamping(const Matrix6d &linear,
                                      const Vector6d &quadratic,
...
void MarineDynamics::configureHydrostatics(double density, double volume,
  ...
  if (!std::isfinite(density) || density <= 0 || !std::isfinite(volume) ||
      volume < 0 || !std::isfinite(gravity) || gravity <= 0 ||
      !cob.allFinite() || !radii.allFinite() || radii.minCoeff() <= 0 ||
      !std::isfinite(level))
...
Matrix6d MarineDynamics::coriolis(const Matrix6d &mass,
...
Vector6d MarineDynamics::acceleration(const Vector6d &v, const Vector6d &vr,
                                      const Vector6d &tau,
...
Vector6d MarineDynamics::dampingWrench(const Vector6d &v) const {
  ...
  return -H.transpose() *
         (linear_damping_ * local +
          (quadratic_damping_.array() * local.array().abs() * local.array())
              .matrix());
}
...
Vector6d
MarineDynamics::restoringWrench(const Eigen::Vector3d &p,
                                const Eigen::Quaterniond &orientation) const {
  const auto q = orientation.normalized();
  Eigen::Vector3d center;
  const double buoyancy =
      density_ * gravity_ * volume_ * submergedFraction(p, q, &center);
  const Eigen::Vector3d force = q.conjugate() * Eigen::Vector3d(0, 0, buoyancy);
  Vector6d tau;
  tau.head<3>() = q.conjugate() *
                  Eigen::Vector3d(0, 0, buoyancy - vehicle_mass_ * gravity_);
...
void MarineDynamics::validateState(const State13d &x) {
  if (!x.allFinite() || x.segment<4>(3).norm() < 1e-10)
    throw std::invalid_argument(
        "State must be finite with a nonzero quaternion");
}
State13d MarineDynamics::derivative(const State13d &x,
                                    const Vector6d &propulsion,
  ...
  const Eigen::Quaterniond q =
      Eigen::Quaterniond(x[3], x[4], x[5], x[6]).normalized();
  ...
  const Eigen::Vector3d pressureForce =
      density_ * volume_ * wet * (q.conjugate() * waterDot);
...
State13d MarineDynamics::step(const State13d &x, const Vector6d &tau, double dt,
  ...
  const State13d k2 =
      derivative(x + dt * .5 * k1, tau, water + dt * .5 * dw, dw);
  const State13d k3 =
      derivative(x + dt * .5 * k2, tau, water + dt * .5 * dw, dw);
