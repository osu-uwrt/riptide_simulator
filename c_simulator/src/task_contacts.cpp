...
V e(const btVector3 &v) { return {v.x(), v.y(), v.z()}; }
btTransform transform(const TaskContacts::Pose &t) {
  Q q(t.rotation());
  return btTransform(btQuaternion(q.x(), q.y(), q.z(), q.w()),
                     b(t.translation()));
}
...
TaskContacts::TaskContacts(const YAML::Node &cfg, const std::string &folder,
                           const V &com,
  ...
  mount.rotate(Eigen::AngleAxisd(p[5].as<double>(), V::UnitZ()) *
               Eigen::AngleAxisd(p[4].as<double>(), V::UnitY()) *
...
void TaskContacts::insert(std::unique_ptr<Entry> x) {
  ...
  x->object.setCollisionFlags(x->kind == 2
                                  ? btCollisionObject::CF_STATIC_OBJECT
                                  : btCollisionObject::CF_KINEMATIC_OBJECT);
  world.addCollisionObject(&x->object, x->kind,
                           x->kind == 1   ? 6
                           : x->kind == 2 ? 5
                                          : 3);
  entries.push_back(std::move(x));
}
void TaskContacts::addBox(const std::string &name, const V &size, const Pose &t,
                          bool vehicle) {
...
void TaskContacts::addMesh(const std::string &name, const std::string &file,
                           const Pose &t, int kind) {
  ...
  if (kind == 2) {
    ...
    for (const auto &f : faces)
      x->triangles->addTriangle(b(vertices.at(f[0])), b(vertices.at(f[1])),
                                b(vertices.at(f[2])));
    x->shape =
        std::make_unique<btBvhTriangleMeshShape>(x->triangles.get(), true);
  } else {
...
void TaskContacts::setJaws(double left, double right) {
  for (auto &x : entries)
    if (x->name == "left" || x->name == "right") {
      ...
      x->local = mount;
      x->local.translate(
          V(0, (x->name == "left" ? 1 : -1) * std::clamp(q, 0., travel), 0));
    }
}
void TaskContacts::setProp(const std::string &name, const Pose &pose,
                           bool attached) {
  for (auto &x : entries)
...
std::vector<TaskContacts::Contact>
TaskContacts::contacts(const Eigen::VectorXd &state) {
  ...
  for (auto &x : entries) {
    x->supported = false;
    if (x->kind != 2) {
      x->object.setWorldTransform(
          transform(x->kind == 1 || x->attached ? body * x->local : x->local));
  ...
  for (int i = 0; i < dispatcher.getNumManifolds(); ++i) {
    ...
    for (int j = 0; j < m->getNumContacts(); ++j) {
      ...
      if (p.getDistance() > .003)
        continue;
      if (a->kind == 4 && c->kind == 2 && c->name != "table" &&
          p.m_normalWorldOnB.z() > .5)
        a->supported = true;
      if (c->kind == 4 && a->kind == 2 && a->name != "table" &&
          p.m_normalWorldOnB.z() < -.5)
        c->supported = true;
    }
  }
  ...
  for (int i = 0; i < dispatcher.getNumManifolds(); ++i) {
    ...
    for (int j = 0; j < m->getNumContacts(); ++j) {
      ...
      if (obstacle->kind == 4 && (!obstacle->supported || normal.z() < .7))
        continue;
      out.push_back({e(av ? p.getPositionWorldOnA() : p.getPositionWorldOnB()),
                     normal, std::max(0., -double(p.getDistance()))});
    }
  }
  return out;
}
Eigen::VectorXd TaskContacts::resolve(Eigen::VectorXd state,
                                      const c_simulator::Matrix6d &inverseMass,
                                      double friction) {
  ...
  for (int pass = 0; pass < 8; ++pass) {
    ...
    for (const auto &c : cs) {
      V n = q.conjugate() * c.normal,
        r = q.conjugate() * (c.point - state.head<3>());
      ...
      if (tangent.norm() > 1e-9) {
        ...
        if (d > 1e-12)
          state.segment<6>(7) -=
              inverseMass * jt *
              std::min(friction * impulse, jt.dot(state.segment<6>(7)) / d);
      }
    }
    if (!changed)
      break;
