...
def payload_mounts(vehicle, task, kind):
    ...
    if kind == 'torpedo':
        ...
        result = [mount @ pose([0, sign*baseline/2, 0, 0, 0, 0]) @ pose(slot)
...
def advance(position, velocity, axis, cfg, water, density, dt):
    ...
    def acceleration(v):
        ...
        drag=-cfg['drag_axial']*np.linalg.norm(axial)*axial-cfg['drag_lateral']*np.linalg.norm(lateral)*lateral
        wet=position[2]<0
        force=drag if wet else np.zeros(3)
        force=force+np.array([0.,0.,9.80665*((density*cfg['displaced_volume'] if wet else 0)-mass)])
        return force/(mass+(cfg['added_mass'] if wet else 0))
    a=acceleration(velocity);b=acceleration(velocity+dt*a/2)
    c=acceleration(velocity+dt*b/2);d=acceleration(velocity+dt*c)
...
def advance_rotating(position, velocity, orientation, angular_velocity, cfg, water, density, dt):
    ...
    inertia_mass = mass + (cfg['added_mass'] if position[2] < 0 else 0)
    inertia = inertia_mass * np.array([
        cfg['radius']**2/2,
        (cfg['length']**2 + 3*cfg['radius']**2)/12 + com**2,
        (cfg['length']**2 + 3*cfg['radius']**2)/12 + com**2])
    state = np.concatenate((position + orientation[:, 0]*com, velocity,
                            orientation.ravel(), angular_velocity))

    def derivative(state):
        center, v = state[:3], state[3:6]
        r, omega = state[6:15].reshape(3, 3), state[15:18]
        axis = r[:, 0]
        wet = (center - axis*com)[2] < 0
        force = np.array([0., 0., -mass*9.80665])
        torque = np.zeros(3)
        if wet:
            buoyancy = np.array([0., 0., density*cfg['displaced_volume']*9.80665])
            drag_arm = axis*(cfg['center_of_drag'] - com)
            relative = v + _cross3(omega, drag_arm) - water
            axial = axis*np.dot(relative, axis)
            lateral = relative - axial
            drag = (-cfg['drag_axial']*np.linalg.norm(axial)*axial
                    - cfg['drag_lateral']*np.linalg.norm(lateral)*lateral)
            force += buoyancy + drag
            torque = (_cross3(axis*(cfg['center_of_buoyancy'] - com), buoyancy)
                      + _cross3(drag_arm, drag)
                      - cfg['angular_damping']*(r @ ((r.T @ omega)*inertia/inertia[1])))
        body_omega = r.T @ omega
        angular_acceleration = r @ ((r.T @ torque - _cross3(body_omega, inertia*body_omega))/inertia)
        # omega cross each orientation column, expressed as a skew matrix.
        wx, wy, wz = omega
        rotation_rate = np.array([[0., -wz, wy], [wz, 0., -wx], [-wy, wx, 0.]]) @ r
        return np.concatenate((v, force/(mass + (cfg['added_mass'] if wet else 0)),
                               rotation_rate.ravel(), angular_acceleration))
...
def crossing(start,end,coordinate,plane=0.):
    """Segment/plane crossing, avoiding high-speed tunneling and repeat hits."""
    a,b=start[coordinate]-plane,end[coordinate]-plane
    if a*b>0 or a==b or a==0:return None
    return start+(end-start)*(-a/(b-a))


def torpedo_contact(start,end,axis,cfg):
    hit=crossing(start,end,0)
    if hit is None or max(abs(hit[1]),abs(hit[2]))>cfg['panel_half_size']:return None
    clearance=cfg['radius']/max(.1,abs(axis[0]))
    for hole in cfg['holes']:
        center=(np.asarray(hole['uv'])-.5)*(2*cfg['panel_half_size'])
