...
class MagnetLights:
    def __init__(self, cfg, frames, vehicle):
        ...
        if not math.isfinite(self.delay) or not 0 < self.delay <= 0.5:
        ...
        self.sensors = {key: (t @ np.r_[offset, 1.0])[:3] for key, t in self.faces.items()}
        mount = pose(vehicle["magnet"]["pose"])
        mount[:3, 3] -= np.asarray(vehicle["base_link"], float)
        self.tip = (mount @ np.r_[tip, 1.0])[:3]
        self.reset()

    def reset(self):
        self.states = self.initial.copy()
        self.dwell = dict.fromkeys(self.initial, 0.0)
    ...
    def step(self, dt, body=None):
        ...
        tip = None if body is None else (body @ np.r_[self.tip, 1.0])[:3]
        activated = []
        for key, sensor in self.sensors.items():
            ...
            if (
                tip is None
                or not np.isfinite(tip).all()
                or np.linalg.norm(tip - sensor) > self.distance
            ):
                self.dwell[key] = 0.0
                continue
            self.dwell[key] += dt
