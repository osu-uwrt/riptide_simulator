...
def main():
    ...
    rho = 998.2
    volume = (
        vehicle["mass"] + 3 / 9.80665
    ) / rho  # 3 N positive buoyancy, an unverified starting assumption
    radii = [
        0.175,
        0.415,
        0.275,
    ]  # existing Talos collision envelope, NOT sealed-volume measurements
    ...
    for i, (_, b, c, d) in enumerate(coefficients):
        v = np.linspace(0, 0.6 if i < 3 else 1.0, 200)
        force = np.maximum(0.0, b * v + c * np.expm1(v / d))
        X = np.column_stack((v, v * v))
        candidates = [
            np.maximum(np.linalg.lstsq(X, force, rcond=None)[0], 0),
            np.array([max(0.0, v @ force / (v @ v)), 0.0]),
            np.array([0.0, max(0.0, (v * v) @ force / ((v * v) @ (v * v)))]),
        ]
        fit = min(candidates, key=lambda x: np.linalg.norm(X @ x - force))
        linear.append(float(fit[0]))
        quadratic.append(float(fit[1]))
    report = dict(
        schema_version=1,
        robot="talos",
        parameter_status="unvalidated_prior",
        ...
        added_mass6x6=added.tolist(),
        linear_damping6x6=np.diag(linear).tolist(),
        quadratic_damping=quadratic,
        damping_center_relative=[0.0, 0.0, 0.0],
        water_density=rho,
        displaced_volume=volume,
        cob_relative=legacy["cob_relative"],
        buoyancy_radii=radii,
        water_level=0.0,
        current_velocity=[0.0, 0.0, 0.0],
        current_oscillation_amplitude=[0.0, 0.0, 0.0],
        current_oscillation_frequency=0.1,
        thruster_dynamics=dict(
            legacy["thruster_dynamics"],
            command_timeout=0.5,
            forward_max_force=legacy["thruster_max_force"],
            reverse_max_force=28.0,
            propeller_radius=0.05,
        ),
        thruster_efficiencies=[1.0] * len(vehicle["thrusters"]),
    )
    Path(args.output).write_text(
        "# UNVALIDATED starting model. See docs/PHYSICS.md before gain transfer.\n"
