...
inline glm::mat4 yamlPose(const YAML::Node &p) {
    ...
    return pose(vector3(p), {p[3].as<float>(), p[4].as<float>(), p[5].as<float>()});
}
...
struct Intrinsics {
    ...
    void validate() const {
        if (width < 16 || height < 16 || width > 4096 || height > 4096 || !std::isfinite(fx) || !std::isfinite(fy) ||
            fx <= 0 || fy <= 0 || !std::isfinite(cx) || !std::isfinite(cy) || !std::isfinite(rate) || rate <= 0 ||
            rate > 120 || !std::isfinite(maxDepth) || maxDepth <= nearPlane)
            throw std::runtime_error("Invalid camera resolution, intrinsics, rate or depth range");
    }
};
inline Intrinsics loadCamera(const std::string &config, const std::string &overrideCalibration, double scale) {
    Intrinsics k;
    ...
    if (!config.empty()) {
        ...
        if (resolution == "HD1200") {
            ...
            throw std::runtime_error("Unsupported camera grab_resolution: " + resolution);
        k.rate = g["pub_frame_rate"].as<double>(g["grab_frame_rate"].as<double>(15));
        if (params["depth"])
            k.maxDepth = params["depth"]["max_depth"].as<double>(8);
        if (calibrationFile.empty())
            calibrationFile = g["optional_opencv_calibration_file"].as<std::string>("");
    ...
    if (!calibrationFile.empty() && std::filesystem::exists(calibrationFile)) {
        ...
        if (!f["K_LEFT"].empty()) {
            ...
            if (kr.empty() || dl.empty() || dr.empty() || r.empty() || t.empty())
                throw std::runtime_error("Stereo calibration requires K/D_LEFT/RIGHT, R, T");
            cv::stereoRectify(K, dl, kr, dr, {w, h}, r, t, r1, r2, P, p2, q, cv::CALIB_ZERO_DISPARITY, 0);
        } else {
            f["image_width"] >> w;
            f["image_height"] >> h;
            ...
            p >> data;
            if (data.size() != 12)
                throw std::runtime_error("Calibration requires rectified projection_matrix.data[12]");
        ...
        throw std::runtime_error("Explicit calibration file does not exist: " + overrideCalibration);
    }
