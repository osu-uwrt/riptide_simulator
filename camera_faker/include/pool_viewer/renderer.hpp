...
struct WaterOptics {
  ...
  float scattering = .10f, distanceScale = 1.f, distancePower = 1.f,
        clearDistance = 0.f;
  void validate() const {
    for (int i = 0; i < 3; ++i)
      if (!std::isfinite(tint[i]) || tint[i] < 0 || tint[i] > 1 ||
          !std::isfinite(absorption[i]) || absorption[i] < 0 || absorption[i] > 5)
        throw std::invalid_argument("Water tint must be in [0,1], absorption in [0,5]");
    if (!std::isfinite(scattering) || scattering < 0 || scattering > 5 ||
        !std::isfinite(distanceScale) || distanceScale < 0 || distanceScale > 5 ||
        !std::isfinite(distancePower) || distancePower < .25f || distancePower > 3 ||
        !std::isfinite(clearDistance) || clearDistance < 0 || clearDistance > 20)
...
class Renderer {
public:
  Renderer(const std::string &shaders, const std::string &meshes,
           const std::string &textures, const std::string &mapping,
           const std::string &markers, const std::string &scene,
           const std::string &robot, const std::string &robotAsset = "",
           const std::string &taskConfig = "",
           const std::string &payloadAsset = "",
           const std::string &launcherAsset = "",
  ...
  void render(Frame &frame, const View &camera, const Look &look, float time,
              bool showRobot = true, bool reflect = false, bool overlays = false);
  // Overlay point cloud: six floats per point (xyz metres, rgb in [0,1]) in
  // the frame that `transform` maps into the map frame.
  void pointCloud(int slot, const std::vector<float> &xyzrgb,
                  const glm::mat4 &transform, const glm::vec3 &highlight);
...
private:
  ...
  GLuint sceneProgram = 0, waterProgram = 0, postProgram = 0, shadowProgram = 0,
         pointProgram = 0, quad = 0;
  std::array<PointCloud, 2> pointClouds;
  ...
  load(const std::string &name, const std::string &overrideTexture = "");
  GLuint texture(const std::string &file);
  void box(const std::string &name, const glm::mat4 &matrix,
           const glm::vec3 &size, const glm::vec3 &color, int material = 0,
           bool castsShadow = true);
  void buildPool();
  void drawScene(const View &camera, const Look &look, float time,
                 bool showRobot, bool clip = false);
