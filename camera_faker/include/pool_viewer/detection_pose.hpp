...
inline bool resolveDetectionPose(const visualization_msgs::msg::Marker &marker,
                                 const std::string &fixedFrame, tf2_ros::Buffer &tf,
                                 glm::mat4 &worldPose,
  ...
  if (acquisitionPose && stamp.nanoseconds() != 0) {
    ...
    try {
      const auto t = tf.lookupTransform(fixedFrame,
          sourceFrame.empty() ? marker.header.frame_id : sourceFrame, stamp).transform;
      const auto &q = t.rotation;
      frame = glm::translate(glm::mat4(1),
...
class DetectionPose {
public:
  bool place(const visualization_msgs::msg::Marker &marker,
             const std::string &fixedFrame, tf2_ros::Buffer &tf,
             const std::string &sourceFrame = "",
             const glm::mat4 *acquisitionPose = nullptr) {
    if (!placed_)
      placed_ = resolveDetectionPose(marker, fixedFrame, tf, world_, sourceFrame,
                                    acquisitionPose);
    return placed_;
  }
