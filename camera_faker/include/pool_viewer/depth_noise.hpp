...
struct DepthNoise {
    ...
    int patchSize = 8;
    void validate() const {
        for (double x : {baseSigma, rangeSigma, exponent, minRange, maxRange, bias, dropout, rangeDropout, edgeDropout,
                         outliers, correlation})
            if (!std::isfinite(x))
                throw std::invalid_argument("Depth settings must be finite");
        if (baseSigma < 0 || rangeSigma < 0 || exponent < 0 || exponent > 4 || minRange <= 0 || maxRange <= minRange ||
            maxRange > 100 || patchSize < 1 || patchSize > 64)
    ...
    void apply(cv::Mat &depth, std::mt19937 &random) const {
        ...
        if (enabled && correlation > 0) {
            cv::Mat coarse((depth.rows + patchSize - 1) / patchSize + 1, (depth.cols + patchSize - 1) / patchSize + 1,
                           CV_32FC1);
        ...
        for (int y = 0; y < depth.rows; ++y) {
            ...
            for (int x = 0; x < depth.cols; ++x) {
                ...
                const double probability =
                    std::clamp(dropout + rangeDropout * std::pow(z / maxRange, 2) + (edge ? edgeDropout : 0), 0., 1.);
                ...
                const double perturbation = independentWeight * independent.at<float>(0, x) + sharedWeight * shared;
