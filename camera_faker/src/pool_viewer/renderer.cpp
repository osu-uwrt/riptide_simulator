...
GLuint program(const std::string &root, const std::string &name) {
  ...
  for (auto type : {GL_VERTEX_SHADER, GL_FRAGMENT_SHADER}) {
    const std::string path =
        root + "/" + name + (type == GL_VERTEX_SHADER ? ".vert" : ".frag");
    std::ifstream f(path);
    if (!f)
...
void uniform(GLuint p, const char *n, const glm::mat4 &v) {
  glUniformMatrix4fv(glGetUniformLocation(p, n), 1, GL_FALSE,
                     glm::value_ptr(v));
}
...
Mesh::Mesh(const std::vector<Vertex> &v, const std::vector<unsigned> &i)
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, i.size() * sizeof(unsigned), i.data(),
               GL_STATIC_DRAW);
...
GLuint Renderer::texture(const std::string &file) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB8_ALPHA8, pixels.cols, pixels.rows, 0,
...
Renderer::load(const std::string &name, const std::string &overrideTexture) {
    ...
    for (unsigned j = 0; j < node->mNumMeshes; ++j) {
      ...
      if (name == "table_pill" || name == "table_nut_and_bolt") {
        ...
        for (size_t k = 0; k < idx.size(); k += 3) {
          ...
          destination.insert(destination.end(), idx.begin() + k,
                             idx.begin() + k + 3);
        }
      ...
      if (name == "torpedo" && idx.size() > 6) {
        ...
        for (size_t k = 0; k < idx.size(); k += 3) {
          ...
          for (size_t corner = 0; corner < 3; ++corner) {
            ...
            panel &= std::abs(p.x) < .01f && std::abs(p.y) <= .30481f &&
                     std::abs(p.z) <= .30481f;
          }
          for (size_t corner = 0; corner < 3; ++corner) {
            if (panel) {
      ...
      if (material->GetTexture(aiTextureType_DIFFUSE, 0, &tex) == AI_SUCCESS) {
        std::filesystem::path t = overrideTexture.empty()
                                      ? std::filesystem::path(tex.C_Str())
...
void Renderer::box(const std::string &name, const glm::mat4 &matrix,
                   const glm::vec3 &size, const glm::vec3 &color, int material,
                   bool castsShadow) {
  auto mesh = cube();
  mesh->color = glm::vec4(color, 1);
  objects.push_back({name,
                     {mesh},
                     glm::scale(matrix, size),
                     material,
                     false,
                     false,
                     castsShadow});
...
void Renderer::buildPool() {
  ...
  const float length = 50, width = 22.86f, depth = 2.1336f, deck = .305288888f;
  auto at = [&](float x, float y, float z) {
    return poolToMap * pose({x, y, z});
  };
  box("Pool floor", at(length / 2, width / 2, -depth - .12f),
      {length, width, .24f}, {.68, .85, .87}, 1);
  box("Near wall", at(length / 2, -.15f, (deck - depth) / 2),
      {length, .3f, depth + deck}, {.68, .85, .87}, 1);
  box("Far wall", at(length / 2, width + .15f, (deck - depth) / 2),
      {length, .3f, depth + deck}, {.68, .85, .87}, 1);
  box("End wall", at(-.15f, width / 2, (deck - depth) / 2),
      {.3f, width, depth + deck}, {.68, .85, .87}, 1);
  box("End wall", at(length + .15f, width / 2, (deck - depth) / 2),
      {.3f, width, depth + deck}, {.68, .85, .87}, 1);
  for (float y : {-1.5f, width + 1.5f})
    box("Deck", at(length / 2, y < 0 ? -1.8f : width + 1.8f, deck - .15f),
        {length + 6.6f, 3, .3f}, {.73, .76, .73}, 2);
  for (float x : {-1.5f, length + 1.5f})
    box("Deck", at(x < 0 ? -1.8f : length + 1.8f, width / 2, deck - .15f),
        {3, width + .6f, .3f}, {.73, .76, .73}, 2);
  for (float y : {-.10f, width + .10f})
    box("Coping", at(length / 2, y, deck + .02f), {length, .22f, .055f},
        {.9, .91, .86});
  for (float x : {-.10f, length + .10f})
    box("Coping", at(x, width / 2, deck + .02f), {.22f, width, .055f},
        {.9, .91, .86});
  ...
  water = {"Water", {surface}, poolToMap};
}
Renderer::Renderer(const std::string &shaders, const std::string &meshes,
                   const std::string &textureFolder, const std::string &mapping,
                   const std::string &markers, const std::string &sceneFile,
                   const std::string &robot, const std::string &robotAsset,
                   const std::string &taskConfig, const std::string &payloadAsset,
                   const std::string &launcherAsset, const std::string &clawAsset)
  ...
  if (!taskConfig.empty()) {
    ...
    for (const auto &h : task["torpedo"]["holes"])
      torpedoHoles.push_back({h["uv"][0].as<float>(), h["uv"][1].as<float>(),
                              h["radius_uv"].as<float>()});
  ...
  buildPool();
  const auto data =
      map["/" + robot + "/riptide_mapping2"]["ros__parameters"]["init_data"];
  if (!data)
    throw std::runtime_error("Mapping configuration has no init_data for " +
                             robot);
  ...
  std::function<glm::mat4(std::string)> resolve = [&](std::string key) {
    ...
    auto p = entry["pose"];
    auto t =
        resolve(entry["parent"].as<std::string>()) *
  ...
  const auto config = YAML::LoadFile(
      markers)["/**/marker_publisher"]["ros__parameters"]["markers"];
  for (const auto &item : config) {
    ...
    if (name == "bin_magnet" && task["magnet_lights"]) {
      ...
      if (lights["targets"][target]) {
        ...
        objects.push_back({target+"/cover",load((folder/"cover.glb").string()),t,5,false,false,false});
  ...
  if (!launcherAsset.empty() && robotAsset.empty())
  ...
  if (!clawAsset.empty() && task["claw"]) {
    ...
    objects.push_back({"Claw right pad", load((std::filesystem::path(clawAsset).parent_path()/"gripper_right.glb").string()), glm::mat4(1)});
  ...
  // legacy.
  auto board = std::make_shared<Mesh>(
      std::vector<Vertex>{{{0, -.3048f, -.4572f}, {1, 0, 0}, {0, 0}},
  ...
  objects.push_back(
      {"Calibration", {board}, pose({.002f, 0, -.345f}), 0, false, true});
...
void Renderer::buildOctagon(const YAML::Node &config) {
  ...
  auto tube = [&](const std::string &name, glm::vec3 a, glm::vec3 b, float r) {
    ...
    const glm::vec3 u = glm::normalize(glm::cross(axis, std::abs(axis.z) < .9f ? glm::vec3(0,0,1) : glm::vec3(0,1,0)));
    ...
    for (int i=0;i<=16;++i) {
      ...
      if(i<16) for(unsigned j:{0u,1u,3u,0u,3u,2u}) indices.push_back(2*i+j);
    }
...
void Renderer::buildCrates(const YAML::Node &config) {
  ...
  const float linerHeight =
      height * config["liner_height_fraction"].as<float>();
  ...
  auto add = [&](int group, const glm::mat4 &t, glm::vec3 size) {
    ...
    for (int axis = 0; axis < 3; ++axis)
      for (int sign : {-1, 1}) {
        ...
        w[(axis + 2) % 3] = sign;
        unsigned start = v.size();
        for (glm::vec2 uv : {glm::vec2(0, 0), glm::vec2(1, 0), glm::vec2(1, 1),
                             glm::vec2(0, 1)})
          v.push_back({glm::vec3(t * glm::vec4((n * .5f + (uv.x - .5f) * u +
                                                (uv.y - .5f) * w) *
                                                   size,
                                               1)),
                       glm::mat3(t) * n, uv});
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    auto part = [&](int material, glm::vec3 p, glm::vec3 size) {
      add(material, root * pose(p), size);
    };
    part(0, {0, 0, -base / 2 - .0005f}, {outer, outer, base});
    for (int axis = 0; axis < 2; ++axis)
      for (int sign : {-1, 1}) {
        auto wall = [&](int material, float across, float along, float z,
                        float thick, float length, float h) {
        ...
        for (int bar = 0; bar <= 8; ++bar)
          wall(0, edge, -outer / 2 + .007f + bar * (outer - .014f) / 8,
               height / 2, .008f, .009f, height);
        for (int bar = 0; bar <= 5; ++bar)
          wall(0, edge, 0, .01f + bar * (height - .02f) / 5, .012f, outer,
               .012f);
        wall(0, edge, 0, height - .008f, .016f, outer, .016f);
        // White corrugated liner covers the lower half only, inside the
        // lattice.
        wall(1, sign * (inner / 2 - liner / 2), 0, linerHeight / 2, liner,
             inner, linerHeight);
      }
  }
  for (int group = 0; group < 2; ++group) {
    ...
    mesh->color =
        group ? glm::vec4(.92, .94, .91, 1) : glm::vec4(.025, .055, .13, 1);
    triangles += indices[group].size() / 3;
    objects.push_back({group ? "Crate liners" : "CleverMade crates",
                       {mesh},
                       glm::mat4(1),
                       group ? 4 : 0});
  }
...
void Renderer::payloadPoses(const std::vector<glm::mat4> &poses) {
  objects.erase(
      std::remove_if(objects.begin(), objects.end(),
                     [](const Object &o) { return o.name == "Payload"; }),
      objects.end());
...
Renderer::~Renderer() {
  ...
  for (GLuint p : {sceneProgram, waterProgram, shadowProgram, postProgram,
                   pointProgram})
    glDeleteProgram(p);
  for (auto &pc : pointClouds) {
    if (pc.vbo) glDeleteBuffers(1, &pc.vbo);
    if (pc.vao) glDeleteVertexArrays(1, &pc.vao);
  }
...
void Renderer::magnetPose(const glm::mat4 &mount) {
  for (auto &o : objects)
    if (o.name == "Robot magnet") o.transform=mount;
}
...
void Renderer::shadows(const Look &look) {
  glm::vec3 center(poolToMap * glm::vec4(25, 11.43, 0, 1));
  auto sun = look.outdoor ? look.sunDirection()
                          : glm::normalize(glm::vec3(-.2f, -.1f, 1));
    ...
    for (const auto &m : o.meshes) {
      ...
      if (!m->holes.empty())
        glUniform3fv(glGetUniformLocation(shadowProgram, "holes"),
                     m->holes.size(), glm::value_ptr(m->holes[0]));
      m->draw();
    }
  }
  glDisable(GL_POLYGON_OFFSET_FILL);
}
void Renderer::drawScene(const View &camera, const Look &look, float time,
                         bool showRobot, bool clip) {
  ...
  uniform(sceneProgram, "sunDirection",
          look.outdoor ? look.sunDirection()
                       : glm::normalize(glm::vec3(-.2f, -.1f, 1)));
  ...
  for (const auto &o : objects) {
    ...
    for (const auto &m : o.meshes) {
      ...
      glUniform4fv(glGetUniformLocation(sceneProgram, "tint"), 1,
                   glm::value_ptr(tint));
...
void Renderer::pointCloud(int slot, const std::vector<float> &xyzrgb,
  ...
  glBindBuffer(GL_ARRAY_BUFFER, pc.vbo);
  glBufferData(GL_ARRAY_BUFFER, xyzrgb.size() * sizeof(float), xyzrgb.data(),
               GL_DYNAMIC_DRAW);
...
void Renderer::clearPointCloud(int slot) { pointClouds.at(slot).count = 0; }
void Renderer::drawPoints(const View &camera, const Look &look) {
  bool any = false;
  for (const auto &pc : pointClouds) any = any || pc.count > 0;
  if (!any) return;
...
void Renderer::render(Frame &f, const View &camera, const Look &look,
                      float time, bool showRobot, bool reflect, bool overlays) {
  glEnable(GL_DEPTH_TEST);
  glDepthMask(GL_TRUE);
  glDisable(GL_BLEND);
  glDisable(GL_CULL_FACE);
  const bool surfaceVisible = look.surface &&
      Frustum(camera.projection * camera.view * water.transform)
          .intersects(water.meshes.front()->bounds);
  const bool reflectionActive = reflect && surfaceVisible && camera.eye.z > 0;
  if (reflectionActive) {
    reflection.resize(std::max(160, f.opaque.width / 2),
                      std::max(100, f.opaque.height / 2));
    ...
    reflected.eye.z = -reflected.eye.z;
  ...
  if (overlays) drawPoints(camera, look);
  glBindFramebuffer(GL_READ_FRAMEBUFFER, f.opaque.fbo);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, f.composite.fbo);
  glBlitFramebuffer(0, 0, f.opaque.width, f.opaque.height, 0, 0, f.opaque.width,
                    f.opaque.height, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT,
                    GL_NEAREST);
  ...
  if (surfaceVisible) {
    ...
    glUniform2f(glGetUniformLocation(waterProgram, "resolution"),
                f.opaque.width, f.opaque.height);
  ...
  uniform(postProgram, "inverseViewProjection",
  ...
  uniform(postProgram, "glare",
          look.outdoor ? look.glare * look.directLight : 0.f);
  glUniform2f(glGetUniformLocation(postProgram, "texel"), 1.f / f.opaque.width,
              1.f / f.opaque.height);
